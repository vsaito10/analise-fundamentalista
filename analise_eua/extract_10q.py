...
from funcoes_sec import (
    ...
    quarterly_income_statement_from_companyfacts,
)
...
def get_balance_sheet(ticker: str) -> pd.DataFrame:
    return _prepare_statement(
        quarterly_balance_sheet_from_companyfacts(ticker.upper())
    )


def get_income_statement(ticker: str) -> pd.DataFrame:
    return _prepare_statement(
        quarterly_income_statement_from_companyfacts(ticker.upper())
    )


def get_cash_flow(ticker: str) -> pd.DataFrame:
    return _prepare_statement(
        quarterly_cash_flow_from_companyfacts(ticker.upper())
    )
...
def build_output_path(ticker: str, output_path: Path | None = None) -> Path:
    ...
    ticker = ticker.lower()
    return Path(__file__).resolve().parent / f"{ticker}_10q_complete.xlsx"


def save_10q(ticker: str, output_path: Path | None = None) -> Path:
